...
An array of LLM judges used to evaluate the agent's performance. Each evaluator becomes its own LLM call per simulation — all evaluators run in parallel against the same conversation transcript.

<Note>
  Simulations have **no implicit defaults**. You must define every evaluator
  yourself, including all of the context (such as the agent's role) that the
  judge needs in its `system_prompt`.
</Note>
...
| `judge_model`   | string  | OpenRouter model id for this evaluator (default: `openai/gpt-5.2`)                                                  |

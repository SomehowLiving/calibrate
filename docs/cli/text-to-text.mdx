...
The simplest setup needs no top-level `evaluators`. Pass `criteria` as a string and the implicit `criteria-passed` evaluator runs. The string is substituted into the default evaluator's system prompt as the `{{criteria}}` variable:
  ...
  "test_cases": [
    {
      ...
      "evaluation": {
        "type": "response",
        "criteria": [
          { "name": "criteria-passed", "arguments": { "criteria": "Politely redirect off-topic questions." } }
        ]
      }
    }
  ]
...
The string-form `criteria` (shown in the quick start above) is just a shortcut for the implicit `criteria-passed` evaluator with `{ "arguments": { "criteria": "<your string>" } }`.
...
<Note>
  The `evaluators` key is optional. When omitted, the implicit `criteria-passed` evaluator is used and `criteria` may be passed as a plain string.
</Note>
